...
class RoutingEmb(Dataloader):
    ...
    def compute_graph(self):
        ...
        # self.t_label_emb = tf.nn.embedding_lookup(self.wemb if self.use_two_matrix else self.emb, self.true_labels)
        # self.s_label_emb = [tf.nn.embedding_lookup(self.wemb if self.use_two_matrix else self.emb, self.samp_labels[:,sid]) for sid in range(self.sample_num)]
        ...
        # Routing
        t_Rout, t_C_IJ = self.routing(t_context_init, t_B_IJ) # shape [B, 2W, 1, VD, 1]

        s_Rout, s_C_IJ = self.routing(s_context_init, s_B_IJ) # shape [B, 2W, S, VD, 1]

        C_IJ = tf.concat([t_C_IJ, s_C_IJ], axis=2) # shape [B, 2W, 1+S, 1, 1]
    ...
    def routing(self, init_input, B_XX):
        ...
        for rt_turn in range(self.routing_iters):
            C_IJ = tf.nn.softmax(B_XX, dim=1, name='C_IJ') # shape [B, 2*W, 1, 1, 1]
            if rt_turn < self.routing_iters - 1:
                S_J = tf.multiply(C_IJ, input_stop, name='S_IJ_in_step') # shape [B, 2*W, S, VD, 1]
                V_J = tf.reduce_sum(S_J, axis=1, keep_dims=True, name='for_V_J') # shape [B, 1, S, VD, 1]
                if self.use_squash:
                    V_J = self.squash(V_J)
                ...
                V_J = tf.reduce_sum(S_J, axis=1, keep_dims=True, name='for_V_J_out')
                if self.use_squash:
                    V_J = self.squash(V_J)
                return V_J, C_IJ  # shape [B, 1, S, VD, 1]
    ...
    def train_epoch(self, epoch_num):
        ...
        for trueL, conT in self.minibatchs:
            ...
            print 'In epoch {}, loss: {:4.4f}\t True_L_acc: {:0.2f}, Sample_L_acc: {:0.2f}\r'.format(epoch_num,
                                    loss, float(accT)/self.batch_size, accS/self.batch_size)
    ...
    def dump_C_IJ(self, contx, true_l, samp_l, C_IJ):
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        TL_savefile = os.path.join(save_path, 'true_label_C_ij.txt')
        SL_savefile = os.path.join(save_path, 'samp_label_C_ij.txt')
        with open(TL_savefile, 'w') as tout, open(SL_savefile, 'w') as sout:
            for sid, (cntx, lblt) in enumerate(zip(contx.tolist(), labels.tolist())):
                cntx = self.idx2words(cntx)
                cntx = ' '.join(cntx[:self.window_size])+' [####] ' + ' '.join(cntx[self.window_size:])
                tLbl = self.idx2words(lblt[0])
                sLbl = self.idx2words(lblt[1:])
                tout.write('TW: '+tLbl+'\tCNTXW: '+cntx+str(C_IJ[sid, :, 0]).replace('\n', '')+'\n')
                for slid, slb in enumerate(sLbl):
                    sout.write('SW: ' + slb + '\tCNTXW: ' + cntx + str(C_IJ[sid, :, slid+1]).replace('\n', '') + '\n')
    ...
    def save_emb(self, name):
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        with open(os.path.join(save_path, 'word_embedding_{}_nor_matrix'.format(self.vector_dim)), 'w') as emb_fot:
    ...
    def _predict(self, analogy):
        ''''''
        idx, = self.sess.run([self._ana_pred_idx], {self._ana_a: analogy[:, 0], self._ana_b: analogy[:, 1],
